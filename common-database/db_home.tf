...
resource "oci_database_db_home" "these" {
  ...
  lifecycle {
    ...
    precondition {
      condition     = each.value.source != "VM_CLUSTER_NEW" ? true : (each.value.vm_cluster_id != null && can(regex("^ocid1\\.(cloudvmcluster|vmcluster|exadbvmcluster)\\.", each.value.vm_cluster_id)))
      error_message = "vm_cluster_id must be a Cloud VM Cluster, Exadata Cloud@Customer VM Cluster, or ExaDB-XS VM Cluster OCID, or a key in vm_cluster_dependency."
    }
