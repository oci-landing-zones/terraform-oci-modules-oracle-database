...
locals {
  common_database_dependency = {
    database_homes      = coalesce(try(var.exadb_xs_dependency.database_homes, null), {})
    databases           = coalesce(try(var.exadb_xs_dependency.databases, null), {})
    pluggable_databases = coalesce(try(var.exadb_xs_dependency.pluggable_databases, null), {})
  }

  common_database_vm_cluster_dependency = merge(
    coalesce(try(var.exadb_xs_dependency.exadb_vm_clusters, null), {}),
    { for key, cluster in oci_database_exadb_vm_cluster.these : key => {
      id             = cluster.id
...
module "common_database" {
  ...
  default_defined_tags              = local.default_defined_tags
  default_freeform_tags             = local.default_freeform_tags
