locals {
  inject_into_existing_vcns = { for k, v in var.autonomous_recovery_service_configuration.recovery_subnets : k => {
    vcn_id = startswith(trimspace(v.vcn_id), "ocid1") ? trimspace(v.vcn_id) : trimspace(var.network_dependency.vcns[trimspace(v.vcn_id)].id)
    network_security_groups = {
      (k) = {
        ...
        display_name   = "${lower(trimspace(v.display_name))}-nsg"
        ingress_rules = {
