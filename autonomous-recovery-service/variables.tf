...
variable "autonomous_recovery_service_configuration" {
  ...
  type = object({
    ...
    enable_iam_policy         = optional(bool, false)
    iam_policy_compartment_id = optional(string)
    ...
    recovery_subnets = optional(map(object({
      ...
      enable_default_nsg = optional(bool, true)       # Indicates whether to enable the default network security group for the recovery service subnet. If set to false, the default network security group is not provisioned for the recovery service subnet.
    ...
    protected_databases = optional(map(object({
      ...
      database_id          = string
      database_size        = string           # The size of the protected database. "XS" - Less than 5GB, "S" - 5GB to 50GB, "M" - 50GB to 500GB, "L" - 500GB to 1TB, "XL" - 1TB to 5TB, "XXL" - Greater than 5TB.
  ...
  validation {
    condition     = alltrue([for database in values(var.autonomous_recovery_service_configuration.protected_databases) : (try(trimspace(database.password), "") != "") != (try(trimspace(database.password_secret_id), "") != "")])
    error_message = "Each protected database must define exactly one of password or password_secret_id."
  }

  validation {
    condition     = alltrue([for database in values(var.autonomous_recovery_service_configuration.protected_databases) : startswith(trimspace(database.protection_policy_id), "ocid1") || contains(keys(var.autonomous_recovery_service_configuration.protection_policies), trimspace(database.protection_policy_id)) || contains(["platinum", "gold", "silver", "bronze"], lower(trimspace(database.protection_policy_id)))])
    error_message = "Each protected database protection_policy_id must be a protection policy OCID, a protection_policies key, or one of Platinum, Gold, Silver, or Bronze."
  }
