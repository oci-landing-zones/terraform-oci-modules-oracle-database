...
resource "oci_recovery_recovery_service_subnet" "these" {
  for_each       = var.rcv_configuration.subnets
  compartment_id = startswith(coalesce(each.value.compartment_id, var.rcv_configuration.default_compartment_id), "ocid1") ? coalesce(each.value.compartment_id, var.rcv_configuration.default_compartment_id) : var.compartments_dependency[coalesce(each.value.compartment_id, var.rcv_configuration.default_compartment_id)].id
  ...
  nsg_ids        = [for n in each.value.nsg_ids : startswith(n, "ocid1") ? n : var.network_dependency.network_security_groups[n].id]
  defined_tags   = coalesce(each.value.defined_tags, var.rcv_configuration.default_defined_tags)
  freeform_tags  = merge(local.cislz_module_tag, coalesce(each.value.freeform_tags, var.rcv_configuration.default_freeform_tags))
}

resource "oci_recovery_protection_policy" "these" {
  for_each                        = var.rcv_configuration.protection_policies
  compartment_id                  = startswith(coalesce(each.value.compartment_id, var.rcv_configuration.default_compartment_id), "ocid1") ? coalesce(each.value.compartment_id, var.rcv_configuration.default_compartment_id) : var.compartments_dependency[coalesce(each.value.compartment_id, var.rcv_configuration.default_compartment_id)].id
  display_name                    = each.value.display_name
...
resource "oci_recovery_protected_database" "these" {
  for_each             = var.rcv_configuration.protected_databases
  compartment_id       = startswith(coalesce(each.value.compartment_id, var.rcv_configuration.default_compartment_id), "ocid1") ? coalesce(each.value.compartment_id, var.rcv_configuration.default_compartment_id) : var.compartments_dependency[coalesce(each.value.compartment_id, var.rcv_configuration.default_compartment_id)].id
  display_name         = each.value.display_name
...
resource "oci_recovery_long_term_backup" "these" {
  for_each                = var.rcv_configuration.long_term_backups
  protected_database_id   = startswith(each.value.protected_database_id, "ocid1") ? each.value.protected_database_id : oci_recovery_protected_database.these[each.value.protected_database_id].id
  display_name            = each.value.display_name
  retention_point_in_time = each.value.retention_point_in_time
  retention_scn           = each.value.retention_scn
  defined_tags            = coalesce(each.value.defined_tags, var.rcv_configuration.default_defined_tags)
  freeform_tags           = merge(local.cislz_module_tag, coalesce(each.value.freeform_tags, var.rcv_configuration.default_freeform_tags))

  retention_period {
    retention_count       = each.value.retention_period_in_days
